...
class UploadFileHelper(BaseFileHelper):
    ...
    async def retrieve_file(self, file: UploadFile, save_path: Path) -> FileRetrievalCodes:
        file_size_mb: float = round(file.size / 2**20, 2)
        file_size_str: str = f"{file_size_mb}mb" if file_size_mb < 1024 else f"{round(file_size_mb / 2**10, 2)}gb"
